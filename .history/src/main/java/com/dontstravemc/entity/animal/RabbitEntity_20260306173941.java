...
public class RabbitEntity extends PathfinderMob implements GeoEntity, Eater, Tetherable {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<RabbitEntity>("controller", 5, state -> {
            ...
            // Handle randomized idles: idle2 every 5-10 regular idles
            if (state.getController().getAnimationState() == software.bernie.geckolib.animation.AnimationController.State.STOPPED) {
                if (state.getController().getCurrentAnimation() != null && state.getController().getCurrentAnimation().animation().name().equals("idle")) {
                    idleCount++;
