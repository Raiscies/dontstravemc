...
public class RabbitEntity extends PathfinderMob implements GeoEntity, Eater, Tetherable {
    ...
    private static final RawAnimation IDLE2_ANIM = RawAnimation.begin().thenLoop("rabbit_idle2");
    private static final RawAnimation WALK_ANIM = RawAnimation.begin().thenLoop("rabbit_walk");
    ...
    private static final RawAnimation LOOK_PRE = RawAnimation.begin().thenPlay("rabbit_look_pre");
    private static final RawAnimation LOOK_LOOP = RawAnimation.begin().thenLoop("rabbit_look_loop");
    private static final RawAnimation HIT_ANIM = RawAnimation.begin().thenPlay("rabbit_hit");
    private static final RawAnimation DEATH_ANIM = RawAnimation.begin().thenPlayAndHold("rabbit_death");
    private static final RawAnimation SLEEP_PRE = RawAnimation.begin().thenPlay("rabbit_sleep_pre");
    private static final RawAnimation SLEEP_LOOP = RawAnimation.begin().thenLoop("rabbit_sleep_loop");
    private static final RawAnimation SLEEP_PST = RawAnimation.begin().thenPlay("rabbit_sleep_pst");
    ...
    private boolean isVegetarian(ItemStack stack) {
        ...
        // Basic list of vegetarian items. Could be expanded with a tag.
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        controllers.add(new AnimationController<>(this, "controller", 5, state -> {
            ...
            if (this.isAlert()) {
                return state.setAndContinue(LOOK_LOOP); // Simplified for now, look_pre needs special handling
            }

            return state.setAndContinue(IDLE_ANIM);
        }).setAnimationModifierHandler(state -> {
            // Handle randomized idles
            if (state.getAnimation() == IDLE_ANIM && state.getController().getAnimationState() == AnimationController.State.STOPPED) {
                idleCount++;
                if (idleCount >= nextSpecialIdle) {
                    idleCount = 0;
                    nextSpecialIdle = 5 + this.random.nextInt(6);
                    return IDLE2_ANIM;
                }
            }
    ...
    public void aiStep() {
        ...
        if (!this.level().isClientSide()) {
            ...
            boolean shouldBeSleeping = shouldReturnHome() && !hasHome(); // Or some other logic if outside
            
            if (shouldBeSleeping && phase == 0) {
                this.entityData.set(SLEEP_PHASE, 1); // Start Pre
            } else if (!shouldBeSleeping && (phase == 1 || phase == 2)) {
                this.entityData.set(SLEEP_PHASE, 3); // Start Pst
            }
            
            // Auto transition phases (simulated timings)
            if (phase == 1 && this.tickCount % 20 == 0) this.entityData.set(SLEEP_PHASE, 2);
            if (phase == 3 && this.tickCount % 20 == 0) this.entityData.set(SLEEP_PHASE, 0);
        }
    }
