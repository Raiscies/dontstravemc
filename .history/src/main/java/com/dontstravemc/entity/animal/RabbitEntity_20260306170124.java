...
public class RabbitEntity extends PathfinderMob implements GeoEntity {
    ...
    public void registerControllers(AnimatableManager.ControllerRegistrar controllers) {
        // Movement Controller: walk, run, idle/idle2
        ...
        // Action Controller: hit, death, eat - higher priority or separate layer
        controllers.add(new AnimationController<>(this, "actions", 2, state -> {
        ...
        // Sleep Controller: sleep_pre -> sleep_loop -> sleep_pst
        controllers.add(new AnimationController<>(this, "sleep", 5, state -> {
            if (this.entityData.get(IS_SLEEPING)) {
                return state.setAndContinue(SLEEP);
            }
            // Logic for sleep_pst (WAKE) could be handled by a transient state if needed
            return PlayState.STOP;
        }));
    }
