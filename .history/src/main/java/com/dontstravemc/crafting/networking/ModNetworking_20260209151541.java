...
public class ModNetworking {
    ...
    private static void handleCraftRequest(ServerPlayer player, ResourceLocation recipeId) {
        ...
        // 1. Validate tech level OR discovery status
        KnowledgeComponents knowledge = ModComponents.KNOWLEDGE.get(player);
        int playerTech = knowledge.getTechLevel();
        boolean discovered = knowledge.isRecipeUnlocked(recipeId);
        
        if (!discovered && playerTech < recipe.getRequiredTechLevel()) return;
