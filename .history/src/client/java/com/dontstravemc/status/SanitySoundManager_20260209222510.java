...
public class SanitySoundManager {
    ...
    private static void startSession() {
        ...
        // Start loop with very low volume (faded in via update)
        currentLoop = SimpleSoundInstance.forUI(loopEvent, 1.0f, 0.01f);
    ...
    private static void updateSession() {
        ...
        if (sessionTicks == 60) {
            float extraVol = 0.6f * currentSanityFactor;
    ...
    private static void startNextWhisper() {
        Minecraft client = Minecraft.getInstance();
        float whisperVol = 0.8f * currentSanityFactor;
