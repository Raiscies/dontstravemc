...
public class SanitySoundManager {
    ...
    public static void tick(Player player, float sanity) {
        ...
        // This is called from HudRenderCallback, so we are on client side.
    ...
    private static void updateSession() {
        ...
        if (sessionTicks == 60) {
            // Volume starts at 0 and fades to 0.6 over 2 seconds (40 ticks)
            currentExtra = SimpleSoundInstance.forUI(ModSounds.CRAZY_EXTRA, 1.0f, 0.0f);
            client.getSoundManager().play(currentExtra);
        }
        
        // Fading crazy extra volume manually if possible (note: requires TickableSoundInstance for real fading)
        // For now, we rely on the sound being registered correctly. 
        // If we want real code-based fading, we'd need a custom SoundInstance class.
        ...
        if (sessionTicks >= loopDurationTicks) {
            ...
            // Set random delay before next session (5-15 seconds)
