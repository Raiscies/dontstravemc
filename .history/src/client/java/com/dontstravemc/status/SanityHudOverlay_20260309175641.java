...
public class SanityHudOverlay implements HudRenderCallback {
    ...
    public void onHudRender(GuiGraphics drawContext, DeltaTracker deltaTracker) {
        ...
        // 只在生存模式和极限模式下显示理智值状态栏
        if (!client.player.getAbilities().instabuild && client.player.isSpectator()) return;
        ...
        // --- 新增：低理智音效系统 ---
        SanitySoundManager.tick(client.player, currentSanity);
        ...
        if (currentSanity < 30.0f) {
            targetAlpha = 1.0f;
            if (!soundPlayed) {
                client.getSoundManager().play(SimpleSoundInstance.forUI(com.dontstravemc.event.ModSounds.MUSIC_DUSK_GONECRAZIER, 1.0f));
                soundPlayed = true;
            }
