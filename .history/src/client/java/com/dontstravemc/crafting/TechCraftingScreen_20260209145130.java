...
public class TechCraftingScreen extends Screen {
    ...
    private void renderCategories(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < categories.length; i++) {
            ...
            if (hasPrototypes) {
                ...
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_RESEARCHABLE_OFF, x - 2, rectY - 3, 0, 0, 16, 16, 16, 16);
            }
    ...
    private void renderRecipeList(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < Math.min(recipes.size(), 64); i++) {
            ...
            if (!techMet || !matsMet) {
                // Fixed: Removed +1 offset to align with grid
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_LOCK_FRAME, rx, ry, 0, 0, 18, 18, 18, 18);
            }
            ...
            if (!techMet) {
                ...
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_LOCK, rx, ry, 0, 0, 18, 18, 18, 18);
            }

            // 4. Render Item icon on top
            // Using +1 offset for items in the 18x18 slot, consistent with vanilla
