...
public class TechCraftingScreen extends Screen {
    ...
    private void renderRecipeList(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < Math.min(recipes.size(), 64); i++) {
            ...
            // Draw 16x16 icon centered in 18x18 slot
    ...
    private void renderRecipeDetails(GuiGraphics guiGraphics, int dx, int dy, int mouseX, int mouseY) {
        ...
        // Labels Background (70% transparent black = 0xB2000000)
        int labelWidth = 111; // 131 - 20
        int lx = dx + 10;
        
        // 1. Name Section
        int ny = dy + 45;
        guiGraphics.fill(lx, ny, lx + labelWidth, ny + 15, 0xB2000000);
        guiGraphics.drawCenteredString(this.font, selectedRecipe.getResult().getHoverName(), dx + 65, ny + 4, 0xFFFFFF);
        
        // 2. Ingredients Section
        int iy = dy + 65;
        guiGraphics.fill(lx, iy, lx + labelWidth, iy + 25, 0xB2000000);
        int j = 0;
        ...
        int descY = dy + 95;
        guiGraphics.fill(lx, descY, lx + labelWidth, descY + 45, 0xB2000000);
        ...
        for (int i = 0; i < Math.min(descLines.size(), 4); i++) {
            guiGraphics.drawString(this.font, descLines.get(i), lx + 5, descY + 5 + (i * 10), 0xCCCCCC, false);
        }
        ...
        guiGraphics.drawCenteredString(this.font, "CRAFT", btnX + 30, btnY + 4, 0xFFFFFF);
    }
