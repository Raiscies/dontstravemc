...
public class TechCraftingScreen extends Screen {
    ...
    private static final ResourceLocation CATEGORY_TEXTURE_BASE = ResourceLocation.fromNamespaceAndPath("dontstravemc", "textures/gui/tech_crafting/categories/");
    ...
    private void renderCategories(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < categories.length; i++) {
            ...
            if (cat == selectedCategory) {
                guiGraphics.fill(x, rectY, x + 30, rectY + 16, 0x66FFFFFF);
            } else if (hovered) {
                guiGraphics.fill(x, rectY, x + 30, rectY + 16, 0x33FFFFFF);
            }
            
            // Render category icon
            ResourceLocation iconTexture = CATEGORY_TEXTURE_BASE.withPath(p -> p + cat.getId() + ".png");
            guiGraphics.blit(iconTexture, x + 7, rectY, 0, 0, 16, 16, 16, 16);
            
            // Render tooltip on hover
            if (hovered) {
                guiGraphics.renderTooltip(this.font, Component.translatable("category.dontstravemc." + cat.getId()), mouseX, mouseY);
            }
        }
    }
