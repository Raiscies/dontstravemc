...
public class TechCraftingScreen extends Screen {
    ...
    private void renderCategories(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < categories.length; i++) {
            ...
            if (hasPrototypes) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_RESEARCHABLE_OFF, x + 3, rectY + 2, 0, 0, 16, 16, 16, 16);
            }
    ...
    private void renderRecipeList(GuiGraphics guiGraphics, int x, int y, int mouseX, int mouseY) {
        ...
        for (int i = 0; i < Math.min(recipes.size(), 64); i++) {
            ...
            if (!techMet || (!matsMet && discovered) || (!matsMet && !discovered)) {
                // User said: "未达到科技和已解锁但材料不足的背景色均为同样的暗颜色"
                // And "lock_itemframe覆盖在背景之上物品之下，用于表示不可合成（无论是否解锁）"
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_LOCK_FRAME, rx + 1, ry + 1, 0, 0, 16, 16, 16, 16);
            }
            
            // 2. If Prototype: render researchable
            if (techMet && matsMet && !discovered) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_RESEARCHABLE, rx + 1, ry + 1, 0, 0, 16, 16, 16, 16);
            }

            // 3. If Locked: render lock icon
            if (!techMet) {
                guiGraphics.blit(RenderPipelines.GUI_TEXTURED, ICON_LOCK, rx + 1, ry + 1, 0, 0, 16, 16, 16, 16);
            }
