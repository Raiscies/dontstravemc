...
public class TrapRenderer extends GeoEntityRenderer<TrapEntity, TrapRenderState> {
    ...
    @Override
    public void actuallyRender(PoseStack poseStack, TrapEntity entity, TrapRenderState state, MultiBufferSource bufferSource, int packedLight, float partialTick) {
        super.actuallyRender(poseStack, entity, state, bufferSource, packedLight, partialTick);
        
        // 渲染诱饵物品
        if (!entity.getBaitItem().isEmpty() && !entity.isTriggered()) {
            poseStack.pushPose();
