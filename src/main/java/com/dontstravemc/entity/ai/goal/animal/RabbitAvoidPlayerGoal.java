...
public class RabbitAvoidPlayerGoal extends Goal {
    ...
    public void tick() {
        ...
        if (this.rabbit.hasHome()) {
            ...
            double distToHome = this.rabbit.distanceToSqr(homePos.getX() + 0.5, homePos.getY(), homePos.getZ() + 0.5);
            
            // 如果距离洞穴小于2格，尝试进入
            if (distToHome < 4.0D) {
                this.tryEnterDen(homePos);
                return;
            }
        }
    ...
    private void tryEnterDen(BlockPos homePos) {
        // 在家的位置搜索洞穴实体
        this.rabbit.level().getEntitiesOfClass(Mob.class,
                        this.rabbit.getBoundingBox().inflate(5.0D),
                        entity -> entity instanceof Den && entity.blockPosition().equals(homePos))
                .stream().findFirst().ifPresent(denEntity -> {
                    ((Den) denEntity).occupantReturned(this.rabbit);
                    this.rabbit.discard(); // 消失，进入巢穴
                });
    }
    ...
    private Vec3 calculateFleeVector() {
        ...
        // 如果有家且是晚上，直接跑回家
        if (this.rabbit.hasHome() && this.rabbit.shouldReturnHome()) {
            BlockPos homePos = this.rabbit.getTether().getHomePos();
            return new Vec3(homePos.getX() + 0.5, homePos.getY(), homePos.getZ() + 0.5);
        }
